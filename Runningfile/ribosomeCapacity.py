...
def mergefigure10(path, exportpath):
    ...
    for pathx in path:
        ...
        # Listeners used
        unique_molecules_reader = TableReader(os.path.join(pathx, "UniqueMoleculeCounts"))
        main_reader = TableReader(os.path.join(pathx, "Main"))
        ribosome_reader = TableReader(os.path.join(pathx, "RibosomeData"))
        ...
        ribosomeCapacity_axis.plot(
            ...
            label="Theoretical total ribosome rate", linewidth=2)
        ribosomeCapacity_axis.plot(
            time / 60., actual_elongation_rate,
            label="Actual elongation rate", linewidth=2)
        ...
        activeRibosomeCapacity_axis.plot(
            ...
            label="Theoretical active ribosome rate", linewidth=2)
        activeRibosomeCapacity_axis.plot(
            time / 60., actual_elongation_rate,
            label="Actual elongation rate", linewidth=2)
        ...
        inactiveRibosomeCapacity_axis.plot(
            ...
            label="Theoretical inactive ribosome rate", linewidth=2)
        ...
        # Save
        plt.tight_layout()
    plt.savefig(exportpath + 'ribosomeCapacity.pdf')
